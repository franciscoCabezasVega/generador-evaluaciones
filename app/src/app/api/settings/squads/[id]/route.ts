...
export async function PATCH(
  ...
  if (body.name !== undefined) {
    ...
    if (productId) {
      const { data: existing } = await supabase
        ...
        .single();
      if (existing) {
        return NextResponse.json({ error: 'Ya existe un squad con ese nombre en este producto' }, { status: 409 });
      }
    }
    updates.name = name;
  }
  ...
  if (error) {
    ...
    return NextResponse.json({ error: 'Error al actualizar squad' }, { status: 500 });
  }
...
export async function DELETE(
  ...
  if (error) {
    ...
    return NextResponse.json({ error: 'Error al eliminar squad' }, { status: 500 });
  }
