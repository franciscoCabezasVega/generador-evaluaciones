...
export async function PATCH(
  ...
  if (body.name !== undefined) {
    ...
    const { data: existing } = await supabase
      ...
      .single();
    if (existing) {
      return NextResponse.json({ error: 'Ya existe una complejidad con ese nombre' }, { status: 409 });
    }
    updates.name = name;
  }
  ...
  if (error) {
    ...
    return NextResponse.json({ error: 'Error al actualizar complejidad' }, { status: 500 });
  }
...
export async function DELETE(
  ...
  if (error) {
    ...
    return NextResponse.json({ error: 'Error al eliminar complejidad' }, { status: 500 });
  }
