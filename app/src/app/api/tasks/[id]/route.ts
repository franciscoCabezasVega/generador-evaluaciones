  ...
  try {
    ...
    if (squads && Array.isArray(squads)) {
      ...
      for (const squadData of squads) {
        if (!validateReturns(squadData.low_returns) || !validateReturns(squadData.medium_returns) || !validateReturns(squadData.high_returns)) {
          return NextResponse.json(
      ...
      const squadsToDelete = existingSquadNames.filter((sq) => !newSquadNames.includes(sq));
      ...
      const upsertRecords = squads.map((squadData: { squad: string; low_returns: number; medium_returns: number; high_returns: number; additional_notes?: string }) => {
    ...
    if ((Array.isArray(updatedSquads) || Array.isArray(existingSquads))) {
      ...
      const oldSquadMap = new Map((existingSquads || []).map((sq) => [sq.squad, sq]));
      const newSquadMap = new Map((updatedSquads || []).map((sq) => [sq.squad, sq]));

      // Conjunto de todos los squads (viejos + nuevos)
      const allSquadNames = new Set([...oldSquadMap.keys(), ...newSquadMap.keys()]);
      ...
      for (const squadName of allSquadNames) {
        ...
        // Si el squad existe en ambas versiones, comparar sus valores
          ...
          const oldLow = oldSquad.low_returns !== null && oldSquad.low_returns !== undefined ? Number(oldSquad.low_returns) : 0;
          const newLow = newSquad.low_returns !== null && newSquad.low_returns !== undefined ? Number(newSquad.low_returns) : 0;
          const oldMedium = oldSquad.medium_returns !== null && oldSquad.medium_returns !== undefined ? Number(oldSquad.medium_returns) : 0;
          const newMedium = newSquad.medium_returns !== null && newSquad.medium_returns !== undefined ? Number(newSquad.medium_returns) : 0;
          const oldHigh = oldSquad.high_returns !== null && oldSquad.high_returns !== undefined ? Number(oldSquad.high_returns) : 0;
          const newHigh = newSquad.high_returns !== null && newSquad.high_returns !== undefined ? Number(newSquad.high_returns) : 0;
          const oldScore = oldSquad.calculated_score !== null && oldSquad.calculated_score !== undefined ? Number(oldSquad.calculated_score) : 0;
          const newScore = newSquad.calculated_score !== null && newSquad.calculated_score !== undefined ? Number(newSquad.calculated_score) : 0;
          ...
          if (lowChanged || mediumChanged || highChanged || scoreChanged || notesChanged) {
...
export async function DELETE(
  ...
  try {
    ...
    if (deleteError) {
      ...
      return NextResponse.json({ error: 'Error al eliminar la tarea' }, { status: 400 });
    }
