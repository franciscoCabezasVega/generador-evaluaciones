      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="space-y-8">
                  ...
                  <div key={squad} className="mb-12">
                    ...
                    {tasks.length === 0 ? (
                      <div className="bg-yellow-50 border border-yellow-200 rounded p-6 mb-6">
                        <p className="text-yellow-800 font-medium">Este equipo no tuvo tareas asignadas en este período.</p>
                      </div>
                    ) : (
                      <div className="bg-gray-50 rounded border overflow-x-auto mb-6">
                        <table className="w-full text-sm">
                          <thead className="bg-gray-100 border-b">
                            <tr>
                              <th className="px-3 py-2 text-left w-12">N° Item</th>
                              <th className="px-3 py-2 text-left w-48">Nombre</th>
                              <th className="px-3 py-2 text-left w-40">Link</th>
                              <th className="px-3 py-2 text-center">Devoluciones Bajas</th>
                              <th className="px-3 py-2 text-center">Devoluciones Medias</th>
                              <th className="px-3 py-2 text-center">Devoluciones Graves</th>
                              <th className="px-3 py-2 text-center">Nota</th>
                            </tr>
                          </thead>
                          <tbody>
                            {tasks.map((task: TaskSquadReportEntry, idx: number) => (
                              <tr key={idx} className="border-b hover:bg-gray-50">
                                <td className="px-3 py-2">{idx + 1}</td>
                                <td className="px-3 py-2 w-48 break-words font-medium">{task.name}</td>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8">
                      {performanceComments[squad] && (
                        <div className="bg-blue-50 p-4 rounded border border-blue-200">
                          <h4 className="font-semibold text-blue-900 mb-2">Desempeño</h4>
                          <p className="text-sm text-blue-800">{performanceComments[squad]}</p>
                        </div>
                      )}
                      ...
                      {communicationComments[squad] && (
                        <div className="bg-green-50 p-4 rounded border border-green-200">
                          ...
                          <p className="text-sm text-green-800">{communicationComments[squad]}</p>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {report.report_data?.deprecatedPendingBySquad && 
            Object.values(report.report_data.deprecatedPendingBySquad).some((tasks: unknown) => Array.isArray(tasks) && tasks.length > 0) && (
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-xl font-semibold mb-6">Tareas Deprecadas y Pendientes</h2>
              
              {sortSquadsByNumber(Object.keys(report.report_data.deprecatedPendingBySquad ?? {})).map((squad) => {
                const tasks = report.report_data.deprecatedPendingBySquad?.[squad];
                if (!tasks || tasks.length === 0) return null;
                
                return (
                  <div key={squad} className="mb-8">
                    <h3 className="font-semibold text-gray-800 mb-4">{squad}</h3>
                    <div className="bg-gray-50 rounded border overflow-x-auto">
                      <table className="w-full text-sm border-collapse">
                        <thead className="bg-gray-100 border-b">
                          <tr>
                            <th className="px-3 py-2 text-left border-r w-12">N° Item</th>
                            <th className="px-3 py-2 text-left border-r w-48">Nombre</th>
                            <th className="px-3 py-2 text-left border-r w-40">Link</th>
                        ...
                        <tbody>
                          {tasks.map((task: TaskSquadReportEntry, idx: number) => (
                            <tr key={idx} className="border-b hover:bg-gray-50">
                              ...
                              <td className="px-3 py-2 border-r w-48 break-words font-medium">{task.name}</td>
