...
export function useFilterParams(defaultFilters: FilterState) {
  ...
  const getFiltersFromUrl = (): FilterState => {
    return {
      month: parseInt(
        searchParams.get("month") || defaultFilters.month.toString(),
      ),
      year: parseInt(
        searchParams.get("year") || defaultFilters.year.toString(),
      ),
      productType:
        searchParams.get("productType") || defaultFilters.productType,
