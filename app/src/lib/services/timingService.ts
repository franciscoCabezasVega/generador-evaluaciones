...
interface TaskIdRow { id: string }
interface TaskProductRow { id: string; product_type: string }
interface TaskInfoRow { id: string; name: string; task_link: string; status: string; product_type: string; created_at: string }
...
function buildDateRangeFilter(
  ...
  query: any,
  filters: { startDate?: string; endDate?: string; month?: number; year?: number }
) {
  // If legacy month/year provided (backward compat), use them directly
  if (filters.month !== undefined && filters.year !== undefined && !filters.startDate) {
  ...
  if (filters.startDate && filters.endDate) {
    ...
    const parseISODate = (dateStr: string): { year: number; month: number; day: number } => {
    ...
    if (startParsed.year === endParsed.year && startParsed.month === endParsed.month) {
      ...
      query = query.eq('year', startParsed.year).gte('month', startParsed.month).lte('month', endParsed.month);
    } else {
      ...
      for (let y = startParsed.year; y <= endParsed.year; y++) {
        ...
        if (minM === maxM) {
          ...
          conditions.push(`and(year.eq.${y},month.gte.${minM},month.lte.${maxM})`);
        }
      }
...
async function getTaskIdsByEffortDate(
  ...
  let q = client.from('tasks').select('id').gte('effort_score_date', startDate).lte('effort_score_date', endDate);
...
async function syncAssignedQA(client: any, taskId: string): Promise<void> {
  try {
    ...
    const qaNames = qaData ? Array.from(new Set(qaData.map((item: { qa_name: string }) => item.qa_name))) : [];
...
// Helper: Get or create task_qa_id for a given task_id + qa_name
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- Supabase client typing
async function getOrCreateTaskQAId(client: any, taskId: string, qaName: string): Promise<string> {
  // Try to find existing task_qa
  ...
  const { data: created, error: createError } = await client
    .from('task_qa')
    .insert({ task_id: taskId, qa_name: qaName })
    .select('id')
    .single();
  ...
  await syncAssignedQA(client, taskId);
...
export const timingService = {
  ...
  async getTimings(filters: {
    ...
    try {
      ...
      if (filters.startDate && filters.endDate) {
        const taskIds = await getTaskIdsByEffortDate(client, filters.startDate, filters.endDate, {
        ...
        if (filters.product_type) {
          ...
          const matchingTaskIds = (matchingTasks || []).map((t: TaskIdRow) => t.id);
      ...
      if (timings.length > 0) {
        ...
        if (!qaError && qaEntries) {
          ...
          for (const entry of qaEntries) {
            ...
            const flatEntry = {
              ...entry,
              qa_name: (entry as { task_qa?: { qa_name: string } }).task_qa?.qa_name || '',
            };
  ...
  async getSquadTimingMetrics(filters: {
    month?: number;
    year?: number;
    startDate?: string;
    endDate?: string;
    ...
    try {
      ...
      let metricsQuery = client.from(TIMINGS_VIEW).select('id, task_id, month, year, effective_testing_hours, waiting_environment_hours, waiting_development_fixes_hours, retest_hours, clarification_hours, total_hours, created_at');
      ...
      const taskIds = [...new Set(timingsData.map((t: TimingViewRow) => t.task_id))];
      ...
      const taskProductTypeMap = (tasksData || []).reduce((acc: Record<string, string>, task: TaskProductRow) => {
        acc[task.id] = task.product_type;
        return acc;
      }, {});

      // Group timings by product_type
      const timingsByProductType = (timingsData as TimingViewRow[]).reduce((acc, timing) => {
        ...
        return acc;
      }, {} as Record<string, { product_type: string; timings: TimingViewRow[] }>);

      // Calculate aggregations
      const metrics: SquadTimingMetrics[] = Object.values(timingsByProductType).map(
        (productData: { product_type: string; timings: TimingViewRow[] }) => {
          ...
          const total_effective_testing_hours = timings.reduce(
            (sum: number, t: TimingViewRow) => sum + (parseFloat(t.effective_testing_hours) || 0),
            0
          );
          const total_waiting_environment_hours = timings.reduce(
            (sum: number, t: TimingViewRow) => sum + (parseFloat(t.waiting_environment_hours) || 0),
            0
          );
          const total_waiting_development_fixes_hours = timings.reduce(
            (sum: number, t: TimingViewRow) => sum + (parseFloat(t.waiting_development_fixes_hours) || 0),
            0
          );
          const total_retest_hours = timings.reduce(
            (sum: number, t: TimingViewRow) => sum + (parseFloat(t.retest_hours) || 0),
            0
          );
          const total_clarification_hours = timings.reduce(
            (sum: number, t: TimingViewRow) => sum + (parseFloat(t.clarification_hours) || 0),
            0
          );
          const total_hours = timings.reduce(
            (sum: number, t: TimingViewRow) => sum + (parseFloat(t.total_hours) || 0),
            0
          );
          ...
          return {
            product_type,
            total_effective_testing_hours: Math.round(total_effective_testing_hours * 100) / 100,
            total_waiting_environment_hours: Math.round(total_waiting_environment_hours * 100) / 100,
            total_waiting_development_fixes_hours: Math.round(total_waiting_development_fixes_hours * 100) / 100,
            total_retest_hours: Math.round(total_retest_hours * 100) / 100,
            total_clarification_hours: Math.round(total_clarification_hours * 100) / 100,
            total_hours: Math.round(total_hours * 100) / 100,
            avg_effective_testing_hours: Math.round((total_effective_testing_hours / task_count) * 100) / 100,
            avg_waiting_environment_hours: Math.round((total_waiting_environment_hours / task_count) * 100) / 100,
            avg_waiting_development_fixes_hours: Math.round((total_waiting_development_fixes_hours / task_count) * 100) / 100,
            avg_retest_hours: Math.round((total_retest_hours / task_count) * 100) / 100,
            avg_clarification_hours: Math.round((total_clarification_hours / task_count) * 100) / 100,
  ...
  async getTaskTimingsWithTaskInfo(filters: {
    ...
    try {
      ...
      const { data: timingsData, error: timingsError } = await query.order('created_at', {
        ascending: false,
      });
      ...
      const tasksMap = (tasksData || []).reduce((acc: Record<string, TaskInfoRow>, task: TaskInfoRow) => {
        acc[task.id] = task;
        return acc;
      }, {});
  ...
  async getQATimingMetrics(filters: {
    month?: number;
    year?: number;
    startDate?: string;
    endDate?: string;
    product_type?: string;
  }, token?: string): Promise<QATimingMetrics[]> {
    try {
      const client = await getClient(token);
      ...
      // When date range is provided, filter by effort_score_date on tasks table
      if (filters.startDate && filters.endDate) {
        const effortTaskIds = await getTaskIdsByEffortDate(client, filters.startDate, filters.endDate, {
          product_type: filters.product_type,
        });
      ...
      let filteredTimingIds = timingsData.map((t: { id: string; task_id: string }) => t.id);
      if (filters.product_type && !(filters.startDate && filters.endDate)) {
        const taskIds = [...new Set(timingsData.map((t: { id: string; task_id: string }) => t.task_id))];
        const { data: tasksData } = await client
          .from('tasks')
          .select('id, product_type')
          .in('id', taskIds)
        ...
        const matchingTaskIds = new Set((tasksData || []).map((t: TaskIdRow) => t.id));
        filteredTimingIds = timingsData
          .filter((t: { id: string; task_id: string }) => matchingTaskIds.has(t.task_id))
      ...
      for (const entry of qaEntries) {
        const qaName = (entry as { task_qa?: { qa_name: string } }).task_qa?.qa_name || '';
        const flatEntry = {
          ...entry,
          qa_name: qaName,
        };
      ...
      const metrics: QATimingMetrics[] = Object.entries(byQA).map(([qaName, entries]) => {
        const totals = {
        ...
        for (const e of entries) {
          totals.effective_testing_hours += Number(e.effective_testing_hours) || 0;
          totals.waiting_environment_hours += Number(e.waiting_environment_hours) || 0;
          totals.waiting_development_fixes_hours += Number(e.waiting_development_fixes_hours) || 0;
        ...
        const efficiencyRate = totals.total_hours > 0
          ? (totals.effective_testing_hours / totals.total_hours) * 100
          : 0;
        const retestRate = totals.effective_testing_hours > 0
          ? (totals.retest_hours / totals.effective_testing_hours) * 100
          : 0;

        return {
          qa_name: qaName,
          total_effective_testing_hours: Math.round(totals.effective_testing_hours * 100) / 100,
          total_waiting_environment_hours: Math.round(totals.waiting_environment_hours * 100) / 100,
          total_waiting_development_fixes_hours: Math.round(totals.waiting_development_fixes_hours * 100) / 100,
          total_retest_hours: Math.round(totals.retest_hours * 100) / 100,
          total_clarification_hours: Math.round(totals.clarification_hours * 100) / 100,
          total_hours: Math.round(totals.total_hours * 100) / 100,
          avg_effective_testing_hours: Math.round((totals.effective_testing_hours / count) * 100) / 100,
          avg_waiting_environment_hours: Math.round((totals.waiting_environment_hours / count) * 100) / 100,
          avg_waiting_development_fixes_hours: Math.round((totals.waiting_development_fixes_hours / count) * 100) / 100,
          avg_retest_hours: Math.round((totals.retest_hours / count) * 100) / 100,
          avg_clarification_hours: Math.round((totals.clarification_hours / count) * 100) / 100,
          avg_total_hours: Math.round((totals.total_hours / count) * 100) / 100,
          task_count: count,
