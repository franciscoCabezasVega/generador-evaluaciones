...
describe('getProductTypeFromSquad', () => {
  it('returns the correct product type for a Platform squad', () => {
    expect(getProductTypeFromSquad('Squad 1 - Alpha', squadsMap)).toBe('Platform');
    expect(getProductTypeFromSquad('Squad 2 - Beta', squadsMap)).toBe('Platform');
  });
  ...
  it('returns the correct product type for a Commerce squad', () => {
    expect(getProductTypeFromSquad('Identity & Auth', squadsMap)).toBe('Commerce');
  ...
  it('returns "Platform" as fallback when the squad is not in the map', () => {
    expect(getProductTypeFromSquad('Unknown Squad', squadsMap)).toBe('Platform');
  });
