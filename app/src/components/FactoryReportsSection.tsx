...
export default function FactoryReportsSection() {
  ...
  // Leer filtros de fábrica desde URL params al cargar el cliente
  useEffect(() => {
    setIsClient(true);
    ...
    setFilters({ month, year, productType, squad });
  ...
  const updateFilters = (newFilters: Partial<typeof filters>) => {
    ...
    if (isClient) {
      startTransition(() => {
        const params = new URLSearchParams();
        // Preservar el tab=fabrica en la URL
        params.set("tab", "fabrica");
        ...
        tasksBySquad[squad] = squadTasks
          .map((task: Record<string, unknown>) => {
            ...
            return {
              ...
              additional_notes: (squadData?.additional_notes as string) || "",
            };
          })
          .filter(
